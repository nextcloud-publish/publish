 ...
 *
 * Which exception is thrown matters as much as whether one is. Messenger checks
 * UnrecoverableExceptionInterface BEFORE consulting the retry strategy, so an
 * unrecoverable throw parks the message immediately while anything else spends
 * the full two-minute budget. A 404 from a client endpoint will not start
 * working on the fifth attempt; a 503 very well might.
 */
final class StatusNotifier
{
    ...
    /**
     * @throws UnrecoverableMessageHandlingException if no retry could succeed
    ...
    public function notify(
        ...
        try {
            $response = $this->httpClient->request('POST', $callbackStatusUrl, [
                ...
                // keep it far inside rabbitmq.conf's consumer_timeout (30s in
                // dev), which would otherwise requeue the message mid-call and
                // turn a slow endpoint into a redelivery.
            ...
            // nature, so let the transport retry them.
