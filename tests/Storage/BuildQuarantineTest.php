...
final class BuildQuarantineTest extends TestCase
{
    ...
    {
        $this->root = sys_get_temp_dir() . '/publish-quarantine-test-' . bin2hex(random_bytes(6));
        ...
        $this->logFile = sys_get_temp_dir() . '/publish-quarantine-log-' . bin2hex(random_bytes(6)) . '.log';
    ...
    {
        $escapee = \dirname($this->root) . '/publish-quarantine-escaped-' . bin2hex(random_bytes(6));
    ...
    public function testQuarantinesAcrossAMountBoundary(): void
    {
        ...
        $this->failedOnOtherMount = '/dev/shm/publish-quarantine-' . bin2hex(random_bytes(6));
