 ...
 * unrecoverable throw parks the message at once while anything else spends the
 * full two-minute budget. Most of this file is about getting that split right.
 *
 * Refusals are proven by the request never being made
 * (getRequestsCount() === 0), not by a mock expectation -- the same idiom
 * ssg-worker's ContentDownloaderTest uses.
 */
final class StatusNotifierTest extends TestCase
{
    ...
    {
        $this->logFile = sys_get_temp_dir() . '/publish-notify-log-' . bin2hex(random_bytes(6)) . '.log';
    ...
    public function testPostsTheAgreedPayloadOnSuccess(): void
    {
        ...
        // The published contract. build_id plus a terminal status is what lets
        // the receiver dedupe, which it has to do: delivery is at-least-once.
        self::assertSame([
            'build_id' => self::BUILD,
